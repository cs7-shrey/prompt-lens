...
interface DataTableProps<TData, TValue> {
  ...
  handleCellClick?: (row: TData) => void
}
...
export function DataTable<TData, TValue>({
  ...
  const showPagination = pageCount !== undefined && pageCount > 1
  const startRow = totalCount ? (currentPage - 1) * pageSize + 1 : 1
  const endRow = totalCount ? Math.min(currentPage * pageSize, totalCount) : data.length
                ...
                table.getRowModel().rows.map((row) => (
                  ...
                  >
                    {row.getVisibleCells().map((cell) => (
                      <TableCell key={cell.id} className="px-4 py-4 hover:cursor-pointer" onClick={() => handleCellClick?.(row.original)}>
      ...
      {showPagination && (
        <div className="flex items-center justify-between px-2 py-4">
          <div className="text-sm text-zinc-500">
            Showing <span className="font-medium text-zinc-400">{startRow}-{endRow}</span> of{" "}
            <span className="font-medium text-zinc-400">{totalCount}</span> responses
          </div>
